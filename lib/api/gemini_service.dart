...
class GeminiService {
  ...
  Future<String> getSummaryForDayEvents(DateTime dateForEvents, List<Event> dailyEvents) async {
   ...
   final String? _apiKey = null;
    ...
    print("GeminiService: API Key status: Successfully loaded from GEM_AI_API_KEY environment variable.");

    print("Requesting summary for: ${DateFormat.yMMMd().format(dateForEvents)}");

    if (dailyEvents.isEmpty) {
      return Future.delayed(const Duration(milliseconds: 100), () => "No events scheduled for this day, so no summary can be generated.");
    }
    ...
    final geminiApiUrl = Uri.parse("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=$_apiKey");
    ...
    try {
      ...
      final response = await http.post(geminiApiUrl, headers: headers, body: body).timeout(const Duration(seconds: 15));
      if (response.statusCode == 200) {
        ...
        String summary = responseData['candidates'][0]['content']['parts'][0]['text'] ?? "Error: Could not parse summary from API response.";
        print("GeminiService: API call successful. Summary received.");
        return summary;
      } else {
        print("GeminiService: API call failed. Status: ${response.statusCode}, Body: ${response.body}");
  ...
  Future<void> sendSpecificEventDetailsToGemini(DateTime eventDate, Event event) async {
    ...
    String promptContent = "Details for a specific event on ${DateFormat.yMMMd().format(eventDate)}:\n";
    final startTimeStr = DateFormat.jm().format(event.startTimeAsDateTime);
    final endTimeStr = DateFormat.jm().format(event.endTimeAsDateTime);
